...
public class WidgetController_PlayerIcon(NetPlayer player, float sizeDelta) : WidgetController
{
    ...
    public override void OnEnable()
    {
        if (VRRigCache.Instance.TryGetVrrig(Player, out playerRig))
        {
            ...
            OnSignificanceChanged(Player,
                    SignificanceManager.Instance.Significance.TryGetValue(Player, out PlayerSignificance[] significance)
                            ? significance
                            : null);
    ...
    private void CreateSubSymbol(ref Image image, Symbols watchSymbol)
    {
        ...
        if (Widget.Object && Widget.Object.transform.Find(objectName) is var child && child &&
            child.TryGetComponent(out Image childImage))
        {
            image = childImage;
