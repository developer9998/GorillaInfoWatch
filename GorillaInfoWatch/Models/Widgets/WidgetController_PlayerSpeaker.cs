...
public class WidgetController_PlayerSpeaker(NetPlayer player) : WidgetController
{
    ...
    public void SetSpeakerState()
    {
        ...
        if (playerRig.Rig.remoteUseReplacementVoice || playerRig.Rig.localUseReplacementVoice ||
            GorillaComputer.instance.voiceChatOn == "FALSE")
        {
            if (playerRig.Rig.SpeakingLoudness > playerRig.Rig.replacementVoiceLoudnessThreshold &&
                !playerRig.ForceMute                                                             && !playerRig.Muted)
            {
        ...
        if (playerRig.Voice != null && playerRig.Voice.IsSpeaking ||
            playerRig.Rig.isLocal   && recorder.IsCurrentlyTransmitting)
        {
            if (!Image.enabled || Image.sprite != open_speaker)
            {
                Image.sprite  = open_speaker;
