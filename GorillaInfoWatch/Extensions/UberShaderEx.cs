...
public static class UberShaderEx
{
    private static readonly string[] supportedShaderNames =
            ["Universal Render Pipeline/Unlit", "Universal Render Pipeline/Lit", "Unlit/Texture", "Custom/UnlitAO",];

    private static string[] keywords;

    private static readonly string[] supportedKeywords =
            ["_USE_TEXTURE", "_WATER_EFFECT", "_HEIGHT_BASED_WATER_EFFECT", "_EMISSION",];

    private static readonly string[] unsupportedKeywords =
            ["_GT_BASE_MAP_ATLAS_SLICE_SOURCE__PROPERTY", "_USE_TEX_ARRAY_ATLAS",];

    private static void GetKeywords()
    {
        if (keywords is not null) return;

        if (VRRigCache.Instance.localRig is RigContainer localRig &&
            localRig.Rig.myDefaultSkinMaterialInstance is Material material)
        {
