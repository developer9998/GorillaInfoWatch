...
public static class PlayerUtility
{
    ...
    public static bool HasInfoWatch(NetPlayer player)
    {
        ...
        if (NetworkSystem.Instance.InRoom && VRRigCache.rigsInUse.TryGetValue(player, out RigContainer playerRig) &&
            playerRig.TryGetComponent(out NetworkedPlayer component))
    ...
    public static void RunScoreboardLineAction(NetPlayer player, Action<GorillaPlayerScoreboardLine, bool> action)
    {
        ...
        [
                .. GorillaScoreboardTotalUpdater.allScoreboards
                                                ...
                                                .Where(line =>
                                                               player.ActorNumber ==
                                                               line.playerActorNumber ||
                                                               player.Equals(line.linePlayer)),
        ];
        ...
        Logging.Warning(
                $"No scoreboard lines detected for player {player.GetName().EnforcePlayerNameLength()} (what the fuck map are you in??)");
    }
}