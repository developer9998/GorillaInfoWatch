...
internal class FriendScreen : InfoScreen
{
    ...
    public async void OnGetFriendsReceived(List<FriendBackendController.Friend> friendsList)
    {
        foreach (FriendBackendController.Friend friend in friendsList)
        {
            ...
            GetAccountInfoResult currentAccountInfo =
                    PlayerExtensions.GetAccountInfo(userId, completionSource.SetResult);
    ...
    public override InfoContent GetContent()
    {
        ...
        lines.Add($"Privacy: {localPlayerPrivacy.GetName().Replace('_', ' ').ToTitleCase()}", new Widget_SnapSlider(
                (int)localPlayerPrivacy, 0, 2, selection =>
                                               {
                                                   localPlayerPrivacy = (FriendBackendController.PrivacyState)selection;
                                                   FriendBackendController.Instance.lastPrivacyState =
                                                           localPlayerPrivacy;
        ...
        {
                Colour = ColourPalette.CreatePalette(ColourPalette.Green.GetInitialColour(),
                        ColourPalette.Red.GetInitialColour()),
        });
        ...
        FriendBackendController.Friend[] friendsList =
                [.. FriendsList.Where(friend => friend is not null).OrderBy(friend => friend.Created),];
        ...
        foreach (FriendBackendController.Friend friend in friendsList)
        {
            ...
            string playerName =
                    ...
                    accountInfo.AccountInfo.TitleInfo.DisplayName        != null            &&
                    accountInfo.AccountInfo.TitleInfo.DisplayName.Length > 4
                            ? accountInfo.AccountInfo.TitleInfo.DisplayName[..^4].EnforcePlayerNameLength()
                            : userName;

            bool isRoomPublic = isPublic.GetValueOrDefault(false);
            bool isOffline    = string.IsNullOrEmpty(roomId) || roomId.Length == 0;
            bool inVirtualStump = !isOffline && (roomId.StartsWith(GorillaComputer.instance.VStumpRoomPrepend) ||
                                                 zoneName != null && zoneName.ToLower()
                                                                             .Contains(GTZone.customMaps.GetName()
                                                                                     .ToLower()));

            bool inZone = !string.IsNullOrEmpty(zoneName) &&
                          ZoneManagement.instance.activeZones.Exists(zone => zoneName.ToLower()
                                                                            .Contains(zone.GetName().ToLower()));
            ...
            string playerStatus = isOffline
                                          ? "Offline"
                                          : inVirtualStump
                                                  ? $"{roomId} in Virtual Stump"
                                                  : isRoomPublic && presence.Zone != null
                                                          ? $"{visibleRoomId} in {presence.Zone.ToUpper()}"
                                                          : visibleRoomId;
            ...
            if ((!isRoomPublic || inZone) && !inVirtualStump)
            {
                ...
                lines.Append(playerName).Append(": ").Append(playerStatus).Add(
                        new Widget_PushButton(JoinFriend, friend, inVirtualStump)
                        {
    ...
    {
        if (args.ElementAtOrDefault(0) is FriendBackendController.FriendPresence presence &&
            args.ElementAtOrDefault(1) is bool isVirtualStump)
        {
            if (isVirtualStump && !GorillaComputer.instance.IsPlayerInVirtualStump())
            {
                ...
                {
                    GameObject treeRoom =
                            ZoneManagement.instance.allObjects.First(gameObject => gameObject.name == "TreeRoom");
            ...
            PhotonNetworkController.Instance.AttemptToJoinSpecificRoom(presence.RoomId,
                    presence.IsPublic.GetValueOrDefault(false)
                            ? JoinType.FriendStationPublic
                            : JoinType.FriendStationPrivate);
        }
    }
}