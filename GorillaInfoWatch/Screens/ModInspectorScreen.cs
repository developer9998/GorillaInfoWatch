...
internal class ModInspectorScreen : InfoScreen
{
    ...
    public override InfoContent GetContent()
    {
        ...
        lines.Add($"State: {(Mod.Instance.enabled ? "<color=green>Enabled</color>" : "<color=red>Disabled</color>")}",
                new Widget_Switch(Mod.Instance.enabled, ToggleMod, Mod));

        List<string> methods = AccessTools.GetMethodNames(Mod.Instance);
        ...
        IEnumerable<ConfigEntryBase> entries = Mod.Instance.Config.Keys.Select(key => Mod.Instance.Config[key]);

        foreach (ConfigEntryBase configEntry in entries)
        {
            Type settingType = configEntry.SettingType;

            if (!configEntry.SettingType.IsEnum && configEntry.SettingType != typeof(int) &&
                configEntry.SettingType                                    != typeof(bool))
                continue;

            lines = new LineBuilder();
            ...
            if (configEntry.SettingType.IsEnum)
            {
                ...
                widgets.Add(new Widget_SnapSlider(
                        Array.IndexOf(names, configEntry.GetSerializedValue()) is int value && value != -1 ? value : 0,
                        0, names.Length - 1, ConfigureEntry, configEntry));
            }
            else if (configEntry.SettingType == typeof(int))
            {
                if (configEntry.Description.AcceptableValues is AcceptableValueRange<int> range)
                    widgets.Add(new Widget_SnapSlider(
                            int.Parse(configEntry.BoxedValue.ToString(), CultureInfo.InvariantCulture), range.MinValue,
                            range.MaxValue, ConfigureEntry, configEntry));
                // TODO: add +/- buttons for adjusting integer
            }
            else if (configEntry.SettingType == typeof(bool))
            {
                widgets.Add(new Widget_Switch(configEntry.BoxedValue.ToString() == "True", ConfigureEntry,
                        configEntry));
            }
        ...
        if (parameters.ElementAtOrDefault(0) is ConfigEntryBase entry)
        {
            if (entry.SettingType == typeof(int))
            {
                ...
                SetText();
                     ...
                     Enum.GetNames(entry.SettingType).ElementAtOrDefault(value) is string enumName)
            {
