...
internal class RoomInspectorScreen : InfoScreen, IInRoomCallbacks
{
    ...
    public override InfoContent GetContent()
    {
        ...
        int maxPlayers = RoomSystem.UseRoomSizeOverride
                                 ? RoomSystem.GetRoomSize(NetworkSystem.Instance.GameModeString)
                                 : PhotonNetwork.CurrentRoom.MaxPlayers;

        lines.Append("Capacity: ")
             .BeginColour(playerCount == maxPlayers ? ColourPalette.Green.GetInitialColour() : Color.white)
             .Append(playerCount).Append(" out of ").Append(maxPlayers).EndColour().AppendLine();
        ...
        lines.Append("Game Mode: ").AppendLine(GameModeUtils.CurrentGamemode is Gamemode gamemode
                                                       ? gamemode.DisplayName
                                                       : GorillaScoreBoard.error.ToTitleCase());
        ...
        if (PhotonNetwork.CurrentRoom.CustomProperties.TryGetValue("queueName", out object queueObject) &&
            queueObject is string queueName)
        {
            bool   isNativeQueue = ComputerQueuePatch.baseGameQueueNames.Contains(queueName);
            string queueTitle    = isNativeQueue ? "Queue" : "Troop";
            lines.Append(queueTitle).Append(": ")
                 .AppendLine(isNativeQueue ? queueName.ToTitleCase() : queueName.ToUpper());
        }

        int participantCount = NetworkSystem.Instance.AllNetPlayers.Where(player => player != null && !player.IsNull)
                                            .Count(GameMode.CanParticipate);

        lines.Append("Participation: ")
             .BeginColour(participantCount == playerCount
                                  ? ColourPalette.Green.GetInitialColour()
                                  : ColourPalette.Red.GetInitialColour()).Append(participantCount).Append(" out of ")
             .Append(playerCount).EndColour().AppendLine();
