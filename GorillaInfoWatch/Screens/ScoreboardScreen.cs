...
internal class ScoreboardScreen : InfoScreen
{
    ...
    public override InfoContent GetContent()
    {
        ...
        lines.Append("In ")
             .Append(configuredPrivacy
                             ? $"Room {NetworkSystem.Instance.RoomName}"
                             : $"{(roomPrivacy ? "Private" : "Public")} Room").Append(": ");

        string gameModeString = NetworkSystem.Instance.GameModeString;
        int maxPlayers = RoomSystem.UseRoomSizeOverride || NetworkSystem.Instance is not NetworkSystemPUN
                                 ? RoomSystem.GetRoomSize(gameModeString)
                                 : PhotonNetwork.CurrentRoom.MaxPlayers;

        lines.Append(NetworkSystem.Instance.RoomPlayerCount).Append("/").Append(maxPlayers).Append(" Players").Add(
                new Widget_PushButton(() => LoadScreen<RoomInspectorScreen>())
                {
        ...
        lines.Append("Game Mode: ").AppendLine(GameModeUtils.CurrentGamemode is Gamemode gamemode
                                                       ...
                                                       : GorillaScoreBoard.error.ToTitleCase()).AppendLine();
        ...
        foreach (NetPlayer player in players)
        {
            ...
            lines.AppendColour(player.GetName().EnforcePlayerNameLength(),
                    GorillaParent.instance.vrrigDict.TryGetValue(player, out VRRig rig)
                            ? rig.playerText1.color
                            : Color.white);
