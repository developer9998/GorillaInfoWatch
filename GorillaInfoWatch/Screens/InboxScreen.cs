...
internal class InboxScreen : InfoScreen
{
    ...
    private readonly string formatOffline = "<line-height=45%><size=50%>{0}</size><br>",
                            formatOnline  = "<line-height=45%><size=50%>{0} in {1}</size><br>";
    ...
    public override InfoContent GetContent()
    {
        ...
        foreach (Notification notification in Contents)
        {
            ...
            string timeDisplay = CountdownText
                                .GetTimeDisplay(timeSpan, "{0} {1} ago", int.MaxValue, "Now", string.Empty).ToString()
                                .ToLower();
            ...
            {
                ConfigEntry<bool> roomPrivacySetting =
                        notification.SessionIsPrivate ? Configuration.ShowPrivate : Configuration.ShowPublic;

                string roomNameProtected = roomPrivacySetting.Value      ? notification.RoomName :
                                           notification.SessionIsPrivate ? "private session" : "public session";
