...
public class HomeScreen : InfoScreen
{
    ...
    internal void SetEntries(List<InfoScreen> screens)
    {
        ...
        IEnumerable<InfoScreen> nativeScreens  = screens.Where(screen => screen.GetType().Assembly == nativeAssembly);
        List<InfoScreen>        orderedScreens = nativeScreens.Concat(screens.Except(nativeScreens)).ToList();

        if (entries.Count > 0) entries.Clear();
        foreach (InfoScreen screen in orderedScreens)
            if (screen.GetType()
                      .GetCustomAttribute<ShowOnHomeScreenAttribute>() is ShowOnHomeScreenAttribute attribute &&
                attribute != null)
            {
                string title =
                        string.IsNullOrEmpty(attribute.DisplayTitle) ||
                        string.IsNullOrWhiteSpace(attribute.DisplayTitle)
                                ? screen.Title
