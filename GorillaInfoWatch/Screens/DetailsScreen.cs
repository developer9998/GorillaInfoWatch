...
[ShowOnHomeScreen]
[PreserveScreenSection(ClearContent = true)]
internal class DetailsScreen : InfoScreen
{
    ...
    public override InfoContent GetContent()
    {
        ...
        GetAccountInfoResult accountInfo =
                NetworkSystem.Instance.GetLocalPlayer().GetAccountInfo(result => SetContent());

        profileLines.Add(
                $"Creation Date: {(accountInfo is null || accountInfo.AccountInfo?.TitleInfo?.Created is not DateTime created ? ". . ." : $"{created.ToShortDateString()} at {created.ToShortTimeString()}")}");
        ...
        if (KIDManager.HasPermissionToUseFeature(EKIDFeatures.Voice_Chat))
        {
            ...
            {
                GorillaComputer.instance.ProcessVoiceState(
                        value ? GorillaKeyboardBindings.option1 : GorillaKeyboardBindings.option2);

                GorillaComputer.instance.UpdateScreen();
                SetContent();
            }));

            profileLines.Add($"Transmission: {GorillaComputer.instance.pttType.ToTitleCase()}", new Widget_SnapSlider(
                    GorillaComputer.instance.pttType switch
                    {
                            "PUSH TO TALK" => 1,
                            "PUSH TO MUTE" => 2,
                            var _          => 0,
                             ...
                             {
                                 if (Enum.TryParse(string.Concat("option", value + 1),
                                             out GorillaKeyboardBindings binding))
                                 {
            ...
            profileLines.Add($"Auto Mute: {GorillaComputer.instance.autoMuteType.ToTitleCase()}", new Widget_SnapSlider(
                    GorillaComputer.instance.autoMuteType switch
                    {
                            "AGGRESSIVE" => 0,
                            "MODERATE"   => 1,
                            var _        => 2,
        ...
        economyLines.Add(
                $"+ 100 Shiny Rocks in: {CountdownText.GetTimeDisplay(whenShinyRocks, "{0} {1}", int.MaxValue, "Now", string.Empty).ToString().ToLower()} ({(DateTime.Now + whenShinyRocks).ToShortTimeString()})");
        ...
        {
            CosmeticsController.CosmeticItem item = currentWornSet.items[i];
        ...
        string accountStatusName = string.IsNullOrEmpty(serializedObject)
                                           ? accountStatus.ToString().ToTitleCase()
                                           : serializedObject.ToTitleCase().Replace('-', ' ').Replace('_', ' ')
                                                             .TrimStart('"').TrimEnd('"');
                ...
                string permissionName = feature switch
                                        {
                                                ...
                                                var _ => permission.Name.ToTitleCase().Replace('-', ' ')
                                                                   .Replace('_', ' '),
                                        };

                string management = permission.ManagedBy == Permission.ManagedByEnum.PROHIBITED
                                            ? "<color=red>Prohibited</color>"
                                            : string.Format("Managed by {0}",
                                                    permission.ManagedBy.ToString().ToTitleCase());
        ...
        progressionLines.Add(
                $"Tutorial Completion: {(NetworkSystem.Instance.GetMyTutorialCompletion() ? "Complete" : "Incomplete")}");
        ...
        if (localRig.TryGetComponent(out GRPlayer grPlayer))
        {
            ...
            (int tier, int grade, int totalPointsToNextLevel, int partialPointsToNextLevel) =
                    GhostReactorProgression.GetGradePointDetails(grPlayer.currentProgression.points);

            int pointsUntilNextGrade = totalPointsToNextLevel - partialPointsToNextLevel;
            progressionLines.Add(
                    $"Employment: {GhostReactorProgression.GetTitleNameAndGrade(grProgression.redeemedPoints)}");

            progressionLines.Add(
                    $"Earned: {partialPointsToNextLevel} out of {totalPointsToNextLevel} / {Mathf.FloorToInt((float)partialPointsToNextLevel / totalPointsToNextLevel * 100f)}%");

            bool readyForPromotion = partialPointsToNextLevel >= totalPointsToNextLevel;
            progressionLines.Add($"Promotion: {readyForPromotion}");
        }
        ...
        */
        ...
        return new PageBuilder(("Profile", profileLines), ("Economy", economyLines), ("Safety", safetyLines),
