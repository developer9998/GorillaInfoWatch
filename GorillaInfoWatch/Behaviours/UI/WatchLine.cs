...
public class WatchLine : MonoBehaviour
{
    ...
    public void Build(InfoLine line, bool applyWidgets)
    {
        ...
        IEnumerable<Widget_Base> widgetsLeftAlign =
                newWidgets.Where(widget => widget.Alignment.Classification == "left");

        float marginLeft = widgetsLeftAlign.Sum(widget => widget.Width);
        marginLeft += widgetsLeftAlign.Any() ? 10 : 0;

        IEnumerable<Widget_Base> widgetsRightAlign =
                newWidgets.Where(widget => widget.Alignment.Classification == "right");
        ...
        for (int i = 0; i < currentWidgets.Count; i++)
        {
            ...
            bool equivalent = currentWidget       != null && currentWidget.Object != null && currentWidget.Object &&
                              newWidget.GetType() == currentWidget.GetType() && newWidget.Equals(currentWidget);
            ...
            if (equivalent)
            {
                ...
                if (newWidget.Alignment.Classification == "custom")
                {
                    ...
                    if (newWidget.Object.transform is RectTransform rectTransform)
                    {
                        ...
                        rectTransform.anchoredPosition = rectTransform.anchoredPosition.WithX(
                                Mathf.Lerp(offset, width - offset, newWidget.Alignment.HorizontalAnchor / 100f) +
                                newWidget.Alignment.HorizontalOffset);

                        rectTransform.localPosition = rectTransform.localPosition.WithZ(
                                newWidget.Controller != null
                                        ? newWidget.Controller.Depth.GetValueOrDefault(newWidget.Depth)
                                        : newWidget.Depth);
                    }
                }
            ...
            else
            {
                ...
                if (newWidget.ControllerType is Type controllerType &&
                    controllerType.IsSubclassOf(typeof(WidgetController)))
                {
                    newWidget.Controller = newWidget.ControllerParameters is object[] parameters
                                                   ? (WidgetController)Activator.CreateInstance(controllerType,
                                                           parameters)
                                                   : (WidgetController)Activator.CreateInstance(controllerType);
            ...
            if (currentWidget.Controller != null
                        ? currentWidget.Controller.Modification.GetValueOrDefault(currentWidget.Modification)
                        : currentWidget.Modification)
                try
                {
