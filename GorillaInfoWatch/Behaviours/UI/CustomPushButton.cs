...
internal class CustomPushButton : MonoBehaviour
{
    ...
    {
        if (!collider.TryGetComponent(out GorillaTriggerColliderHandIndicator handIndicator) || handIndicator.isLeftHand == Watch.LocalWatch.InLeftHand || !WatchManager.Instance.CheckInteractionInterval(WatchInteractionSource.Screen, Active ? Debounce : 0.05f)) return;
