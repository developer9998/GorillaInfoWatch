...
public class SignificanceManager : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        CosmeticsV2Spawner_Dirty.OnPostInstantiateAllPrefabs2 +=
                () => CheckPlayer(NetworkSystem.Instance.GetLocalPlayer(), SignificanceCheckScope.Item);
    }

    public async void Start()
    {
        if (NetworkSystem.Instance is not NetworkSystem netSys ||
            PlayFabAuthenticator.instance is not PlayFabAuthenticator authenticator) return;
    ...
    public void OnPlayerJoined(NetPlayer player)
    {
        ...
        {
            Notifications.SendNotification(new Notification("Your friend has joined",
                    string.Format("<color=#{0}>{1}</color>", ColorUtility.ToHtmlStringRGB(GFriends.m_clrFriend),
                            player.GetName().EnforcePlayerNameLength()), 3f, Sounds.notificationPositive,
                    new Notification.ExternalScreen(typeof(PlayerInspectorScreen),
                            $"Inspect {player.GetName().EnforceLength(12)}", delegate
                                                                             {
                                                                                 player = PlayerUtility.GetPlayer(
                                                                                         userId);

                                                                                 if (player != null && !player.IsNull)
                                                                                     PlayerInspectorScreen.UserId =
                                                                                             player.UserId;
                                                                             })));
        ...
        {
            Notifications.SendNotification(new Notification("A verified user has joined",
                    string.Format("<color=#{0}>{1}</color>", ColorUtility.ToHtmlStringRGB(GFriends.m_clrVerified),
                            player.GetName().EnforcePlayerNameLength()), 3f, Sounds.notificationPositive,
                    new Notification.ExternalScreen(typeof(PlayerInspectorScreen),
                            $"Inspect {player.GetName().EnforceLength(12)}", delegate
                                                                             {
        ...
        if (Significance.TryGetValue(player, out PlayerSignificance[] significance) &&
            significance.Any(item => item is FigureSignificance))
            Notifications.SendNotification(new Notification("A notable user has joined",
                    player.GetName().EnforcePlayerNameLength(), 3f, Sounds.notificationPositive,
                    new Notification.ExternalScreen(typeof(PlayerInspectorScreen),
                            $"Inspect {player.NickName.SanitizeName()}", delegate
                                                                         {
    ...
    public void OnPlayerLeft(NetPlayer player)
    {
        ...
        if (GFriends.IsFriend(userId))
            Notifications.SendNotification(new Notification("Your friend has left",
                    string.Format("<color=#{0}>{1}</color>", ColorUtility.ToHtmlStringRGB(GFriends.m_clrFriend),
                            player.GetName().EnforcePlayerNameLength()), 5, Sounds.notificationNegative));
        else if (GFriends.IsVerified(userId))
            Notifications.SendNotification(new Notification("A verified user has left",
                    string.Format("<color=#{0}>{1}</color>", ColorUtility.ToHtmlStringRGB(GFriends.m_clrVerified),
                            player.GetName().EnforcePlayerNameLength()), 5, Sounds.notificationNegative));
        else if (Significance.TryGetValue(player, out PlayerSignificance[] significance) &&
                 significance.Any(item => item is FigureSignificance))
            Notifications.SendNotification(new Notification("A notable user has left",
                    player.GetName().EnforcePlayerNameLength(), 5, Sounds.notificationNegative));
    ...
    public void OnGetUserCosmetics(VRRig rig)
    {
        ...
        if (CheckPlayer(player, SignificanceCheckScope.Item)                        &&
            Significance.TryGetValue(player, out PlayerSignificance[] significance) &&
            Array.Find(significance, item => item is ItemSignificance) is ItemSignificance item)
        {
            string userId = player.UserId;
            string displayName =
                    CosmeticsController.instance.GetItemDisplayName(
                            CosmeticsController.instance.GetItemFromDict(item.ItemId));

            Notifications.SendNotification(new Notification("A notable cosmetic was detected", displayName, 5,
                    Sounds.notificationPositive, new Notification.ExternalScreen(typeof(PlayerInspectorScreen),
                            $"Inspect {player.NickName.SanitizeName()}", delegate
                                                                         {
                                                                             player = PlayerUtility.GetPlayer(userId);
                                                                             if (player != null && !player.IsNull)
    ...
    public bool CheckPlayer(NetPlayer player, SignificanceCheckScope checkScope)
    {
        ...
        if (player.IsNull)
            throw new ArgumentException("NetPlayer is classified as null (NetPlayer.IsNull)", nameof(player));

        PlayerSignificance[] array = Significance.ContainsKey(player)
                                             ? [.. Significance[player],]
                                             : [.. Enumerable.Repeat<PlayerSignificance>(null, 5),];

        if (checkScope.HasFlag(SignificanceCheckScope.RemovalCandidate) && Significance.ContainsKey(player) &&
            !player.IsLocal && (!NetworkSystem.Instance.InRoom || !player.InRoom))
        {
        ...
        if (checkScope.HasFlag(SignificanceCheckScope.Figure))
            array[0] =
                    Array.Find(Main.Significance_Figures.ToArray(), figure => figure.IsValid(player)) is
                            FigureSignificance figure
                            ? figure
                            : null;
        ...
        if (checkScope.HasFlag(SignificanceCheckScope.Item))
            array[2] =
                    Array.Find(Main.Significance_Cosmetics.ToArray(), cosmetic => cosmetic.IsValid(player)) is
                            ItemSignificance cosmetic
                            ? cosmetic
                            : null;

        if (checkScope.HasFlag(SignificanceCheckScope.InfoWatch) && !array.Contains(Main.Significance_Watch) &&
            PlayerUtility.HasInfoWatch(player))
